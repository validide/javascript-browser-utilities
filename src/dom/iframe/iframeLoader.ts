...
export class IframeLoader extends BaseComponent {
  ...
  constructor(window: Window, options: IframeLoaderOptions) {
    super(window);
    if (!options?.url) throw new Error('The "options.url" value should be a non-empty string.');
  ...
  private getParentElement(): HTMLElement {
    ...
    if (!parent) throw new Error(`Failed to find parent "${opt.parent}".`);
  ...
  private triggerEvent(eventType: IframeLoaderEventType): void {
    const opt = this.getOptions();
    const handler = opt.events ? opt.events[eventType] : undefined;
  ...
  private windowMessageHandler(event: MessageEvent): void {
    if (event.origin !== this.getIframeOrigin()) return;

    const messageData = event.data ? (event.data as IframeMessage) : null;
    ...
    if (!messageData.id || messageData.id !== this.iframeId) return;
  ...
  private shouldShakeHands(message: IframeMessage): boolean {
    ...
    if (!message.id && message.state === IframeMessageState.Mounted) return true;
  ...
  private shakeHands(requestMessage: IframeMessage): void {
    ...
    ((this.getIframe() as HTMLIFrameElement).contentWindow as Window).postMessage(
      responseMessage,
...
export class IframeContent extends BaseComponent {
  ...
  constructor(window: Window, parentOrigin: string) {
    ...
    this.iframeId = "";
    this.messageQueue = [] as IframeMessage[];
    this.disposed = false;
    if (this.standalone) {
      this.onMessageReceived = null;
    } else {
  ...
  public signalBusyState(busy: boolean): void {
    this.sendMessage({
      id: this.iframeId,
      state: busy ? IframeMessageState.BeforeUpdate : IframeMessageState.Updated,
    });
  }
